...
export default function Profile() {
  ...
  return (
    <>
      <div className="mx-auto max-w-7xl py-3.5">
        <main className='mun-w-0'>
                        ...
                        <div className="rounded-3xl border border-white/60 bg-white/92 p-5 backdrop-blur-xl sm:p-7">
                            ...
                            <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
                                        ...
                                        <div className="group/avatar relative shrink-0">
                                            <button type='button' className='cursor-zoom-in rounded-full'>
                                                <img src={profileData?.photo}
                                                 alt={profileData?.name || "Unknown"} className='h-28 w-28 rounded-full border-4 border-white object-cover shadow-md ring-2 ring-[#dbeafe] '/>
                                            </button>

                                            <button
                                            title='view profile Photo'
                                            type="button" className="absolute bottom-1 left-1 flex h-9 w-9 cursor-pointer items-center justify-center rounded-full bg-white text-[#1877f2] opacity-100 shadow-sm ring-1 ring-slate-200 transition duration-200 hover:bg-slate-50 sm:opacity-0 sm:group-hover/avatar:opacity-100 sm:group-focus-within/avatar:opacity-100">
                                                <LuExpand/>
                                            </button>
                                ...
                                <div className="grid w-full grid-cols-3 gap-2 lg:w-130">
                                    ...
                                    <div className="rounded-2xl border border-slate-200 bg-white px-3 py-3 text-center sm:px-4 sm:py-4">
                                        <p className='text-[11px] font-bold uppercase tracking-wide text-slate-500 sm:text-xs'>Folloing</p>
                                        <p className="mt-1 text-2xl font-black text-slate-900 sm:text-3xl">0</p>
                                    </div>
                            ...
                            <div className="mt-5 grid gap-4 lg:grid-cols-[1.3fr_.7fr]">
                                ...
                                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                                    ...
                                    <div className="mt-3 space-y-2 text-sm text-slate-600">
                                        <p className="flex items-center gap-2">
                                            <MdMailOutline className='text-slate-500'/>
                                            {profileData?.email}
                                        </p>
                                        <p className="flex items-center gap-2">
                                            <LuUsers className='text-slate-500'/>
                                            Active on Routes Posts
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
                                    <div className="rounded-2xl border border-[#dbeafe] bg-[#f6faff] px-4 py-3">
                                        ...
                                        <p className="mt-1 text-2xl font-black text-slate-900">20</p>
                                    </div>
                                    <div className="rounded-2xl border border-[#dbeafe] bg-[#f6faff] px-4 py-3">
                ...
                <section className='space-y-4'>
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
                        <div className="grid w-full grid-cols-2 gap-2 rounded-xl bg-slate-100 p-1.5 sm:inline-flex sm:w-auto sm:gap-0">
                            <button className='inline-flex  items-center justify-center rounded-lg px-4 gap-2 py-2 text-sm font-bold transition bg-white text-[#1877f2] shadow-sm'>
                                <FiFileText/>
                                My Posts
                            </button>
                            <button className='inline-flex items-center justify-center rounded-lg px-4 gap-2 py-2 text-sm font-bold transition text-slate-600 hover:text-slate-900'>
                                <CiBookmark/>
                                Saved
                            </button>
                        </div>
                        <span className='rounded-full bg-[#e7f3ff] px-3 py-1 text-xs font-bold text-[#1877f2]'>20</span>
                    </div>
